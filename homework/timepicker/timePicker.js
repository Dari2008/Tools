var tp = {
    ...
    maxhr : 24,   // max spin limit for hour
    minmin : 0,   // min spin limit for minute
    maxmin : 55,  // max spin limit for minute
    spin : (direction, segment) => {
      ...
      if (direction==null) { 
        ...
        tp.incr(direction, segment)
  
        // (B3-4) KEEP ROLLING - LOWER TIMEOUT WILL SPIN FASTER 
        tp.timer = setTimeout(() => tp.spin(direction, segment), 200);
      }
    },

    incr: function(direction, segment){
      let next = +tp["h"+segment].innerHTML;

      if(segment == "min"){
        next = direction ? next+5 : next-5;
      }else{
    ...
    set : () => {
      // (E1) TIME TO FIELD
      if (tp.set24) {
        tp.setfield.innerHTML = tp.hhr.innerHTML + ":" + tp.hmin.innerHTML;
      } else {
        tp.setfield.innerHTML = tp.hhr.innerHTML + ":" + tp.hmin.innerHTML + " " + tp.hap.innerHTML;
      }
      tp.hwrap.classList.remove("show");
  
      // (E2) RUN AFTER, IF SET
      if (tp.setafter) { tp.setafter(tp.setfield.innerHTML);}
    }
  };
